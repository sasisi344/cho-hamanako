...
- **Sub-categories** :
    ...
    - 奥浜名湖: `oku`
    - 月別おすすめ情報: `monthly`
    - 深掘り攻略（Expert）: `fukabori` # クラスターコンテンツ
...
### 3. 時期・シーズン情報 (Season & Monthly)
- **Slug** : `season` または `monthly`
- **Description** : 季節や月ごとの「今何が釣れるか？」「どんなパターンか？」を中心とした情報。
- **Sub-categories** :
    - 季節別: `spring`, `summer`, `autumn`, `winter`
    - 月別: `january`, `february`, ..., `september` など

### ※ 廃止・移行したカテゴリ：魚種別攻略 (Target Fish)
以前の `target` ディレクトリは**廃止**。メインKW（検索意図）をカテゴリとするため、特定の魚種に関する攻略記事は `guide` （テクニック・入門）または `season` （時期）に割り当て、**魚種名はすべてタグで管理する**。
...
- **Target Fish (魚種)** : `haze` (ハゼ), `seabass` (シーバス), `kurodai` (クロダイ), `kiss` (キス), `aji` (アジ) など
- **Location Name (ポイント名)** : `aboshiba` (網干場), `maisaka` (舞阪) 等。**【深掘り記事は必須】** 関連記事の制御に使用。
...
### Basic Format
`https://cho-hamanako.info/[category-slug]/[article-slug]`
...
- **ポイント（ピラー）**: `points/bentenjima-park-guide`
- **ポイント（深掘り）**: `points/fukabori/[point-name]-area-fukabori`
