...
2. **SEOカニバリズム防止**: 直近の記事作成では、既存WPサイトとの競合を防ぐために必ず `noindex: false` を付与する。
3. **柔軟な真偽値管理**: 施設有無などは `true` / `false` だけでなく、"場所による" などの詳細な補足（string）も許容する仕組みとする。
10: 2. **SEOカニバリズム防止**: 直近の記事作成では、既存WPサイトとの競合を防ぐために必ず `noindex: false` を付与する。
11: 3. **真偽値（Boolean）の活用**: フィルタリングの高速化のため、施設有無などは可能な限り `true` / `false` で管理する。
...
| `lat` | float | **緯度（任意）** | Google Mapsから取得 |
| `lng` | float | **経度（任意）** | Google Mapsから取得 |
...
| プロパティ名 | 型 | 説明 | 備考 |
| :--- | :--- | :--- | :--- |
| `difficulty` | string | **難易度** | `Beginner`, `Intermediate`, `Advanced`, `Medium`, `Hard` など |
...
| プロパティ名 | 型 | 説明 | 備考 |
| :--- | :--- | :--- | :--- |
| `parking` | bool/str | 駐車場の有無 | `true`, `false`, "有料Pあり" など |
| `parkingFee` | string | 駐車料金 | "無料"、"500円/回" など |
| `toilet` | bool/str | トイレの有無 | `true`, `false`, "公衆トイレあり" など |
| `convenienceStore`| string | コンビニの有無/距離 | "近隣に有り"、"車で5分" など |
| `nightFishing` | bool/str | 夜釣りが可能か | `true`, `false`, "禁止箇所あり" など |
| `streetLights` | bool/str | 常夜灯の有無 | `true`, `false`, "場所による" など |
| `carSide` | bool/str | **車横付け可能** | `true`, `false`, "一部可能" など |
...
5. **画像生成プロンプトによる「リズム」の創出**
    - 記事の可読性を高めるため、適切な位置に **画像生成プロンプト** を MDX コメント形式（`{/* ... */}`）で挿入する。詳細は `.agents/writing-style/SKILL.md` を参照。
