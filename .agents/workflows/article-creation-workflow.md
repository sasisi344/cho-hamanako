...
- **ドラフト確認**: `_draft/` 内の対象ファイルを読み、SEOターゲットと構成を把握する。
...
> 「`.agents/workflows/article-creation-workflow.md` に従って、`_draft/draft-sayori-april.md` を完成させて。リサーチデータは `.data-set/research/biological/aori-ika.md`（※対象のもの）を参考にして。**必ずBlogCard and TackleCardのリンク先が実在することを確認して**。画像生成までお願い。」
