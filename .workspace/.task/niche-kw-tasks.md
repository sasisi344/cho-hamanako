...
- [ ] **「浜名湖 電車 釣り おすすめ 4選」**（**新規**）: 駅から徒歩圏内のポイントを横断比較する集約記事。各ポイント記事（弁天島・佐久米・寸座・三ヶ日など）に分散している情報のハブ役。

### ⚓ 経験者 & 深掘りユーザー向け (Experienced/Core)

- [ ] **「浜名湖 [特定ポイント] 激流 攻略法」**（**リライト・追記**）: 新規単体記事は作らない。既存の今切口・弁天島系記事へ下げ潮・流しの節を追記する形で対応。
...
- [ ] **「観光名所 × 釣り」サジェストKWの抽出**（**調査タスク**）: ラッコキーワード等から観光スポット（弁天島、ガーデンパーク、はまゆう、みかん、うなぎ等）×「釣り」の組み合わせを洗い出す。公開記事化は `travel/hamanako-kanko-hub` の拡張と役割分担を決めてから。

- [ ] **「特産グルメ × 釣り場」記事**（**要差別化・低優先**）: 季節別旅行記事（牡蠣・しらす等）と被らないよう、「グルメ名所から徒歩・車○分の釣り座」マップ型に限定。未着手のままなら kanko-hub のグルメ節への追記でも可。
...
- [x] `src/content/blog/points/` 以下に「電車」「レンタル」の記載あり（弁天島・佐久米・寸座・三ヶ日・新居弁天・ボートレンタル各所など）。**集約記事「電車4選」のみ未整備**。
...
- [ ] **フェーズ1**: **新規**「電車×釣り4選」の公開。レンタル・手ぶらは `hamanako-rental-fishing-guide` のリライト（料金・季節）で対応。
- [ ] **フェーズ2**: 激流・夜釣りは **新規記事を増やさず**、今切口深掘り・電気ウキガイド・ナイトシーバス記事への追記と相互リンク。
- [ ] **フェーズ3**: 全記事から `travel/hamanako-kanko-hub` および各エリアピラーへ BlogCard を設置（特に除外したKW群→既存ガイドへの導線）。
