...
- `cover.jpg取得`: （アイキャッチ画像URL）DLしてpagebundleのcover.jpgに設定
...
- **アイキャッチ＆インフォグラフィックの配置**: 
  - メタデータの `cover.jpg取得` に記載されているWordPressのアイキャッチ画像URLは、**必ずダウンロードして記事のPage Bundleフォルダ内に格納**し、フロントマターで `cover: "./cover.jpg"` のように相対ファイルパスとして再設定してください。
  - 記事内には、アイキャッチかインフォグラフィックのどちらか必ず1枚は使うこと。画像生成を行う場合は、プロジェクト専用スクリプト `skills/scripts/Antigravity-nanobana/generate-image.js` の使用を最優先してください。
  - listは5行以上なら画像とみなしてよいです。アイキャッチにはテキストを使わないこと。インフォグラフィックには日本語テキストを使うこと。

### Step 6: 保存先ディレクトリへの提案・出力 (Page Bundle Format)
処理とリライトが完了したコンテンツは、必ずAstroの **Page Bundle（ページバンドル）形式** に対応するように出力・配置します。
具体的には、記事単体のMarkdownファイルを置くのではなく、**記事のスラッグ名でフォルダを作成し、その中に `index.mdx` として保存**します（関連する画像等も同じフォルダ内に格納します）。

- **本番用**: `src/content/blog/{category}/{slug}/index.mdx`
- **ドラフト（作業用）から開始した場合**: `skills/guidelines/draft/{category}/{slug}/index.mdx`

### Step 7: 使用済みドラフトのアーカイブ
`skills/guidelines/draft/` にあるドラフトファイルを元に本記事を作成（Astro形式に変換）した後は、必ず元のドラフトファイルを `skills/guidelines/.archieve/` ディレクトリへ移動させてください。
基本的に、ドラフトから記事生成を完了したものはすべてこのアーカイブ用フォルダへ格納し、作業中フォルダ（`draft/`）を常にクリーンに保つことをルールとします。
