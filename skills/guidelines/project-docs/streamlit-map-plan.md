...
### Phase 1: データ整備
- [x] 浜名湖の主要ポイントの座標（緯度・経度）リストをCSV/JSON化。
- [x] カテゴリやタグ（ハゼ、駐車場あり等）の属性情報を紐付け。

### Phase 2: Streamlitアプリ開発
- [x] 全ピン表示機能の実装。
- [x] URLパラメータによる中心位置・ズーム制御の実装（Astro側との連携基盤完了）。
- [x] サイドバーによる「魚種別・施設別フィルタ」の実装。

### Phase 3: Astroへの組み込み
- [x] 個別記事コンポーネント（Map.astro等）の作成。
- [x] オンデマンドロード（ボタンクリックでiframe表示）のJS実装。
  - `StreamlitMapButton.astro` コンポーネントとして実装済み。
  - URLパラメータ（`lat`, `lng`, `zoom`, `embed`）でStreamlitアプリと連携。
